...
public class GPTChatRestService {
	...
	public GPTResponseDto completionChat(String content) {
		...
		messages.add(new ChatMessage("system",
				"너는 문자열을 분석해서 json타입으로 변환하는 데이터 분석 프로그램이야. json의 key 값은 'action', 'name', 'amount' 가 있어. 'action'의 value는 ['송금', '조회', '즐겨찾기', '예', '아니오'] 중 한 가지야. 오타가 존재할 수 있으니 유사한 단어로 이해해 줘. 'action'이 '조회'나 '즐겨찾기'  또는 '예', '아니오'일 때 'name'은 공백으로 주고, 'amount' 는 0으로 하면 돼. 예를 들어 '김건에게 만원 송금해줘' 라는 입력을 받으면 { \"action\" : \"송금\", \"name\" : \"김건\", \"amount\" : 10000}  이렇게 응답하면 돼."
		...
		try {
			...
			gptResponseDto = new GPTResponseDto("etc", " ", bigDecimal);
